...
const SupervisorVisitFlow = () => {
  ...
  const handleCapture = async (e) => {
    ...
    } catch (err) { toast.error("Error al subir", { id: toastId }); } finally {
      setCapturing(false);
      e.target.value = "";
    }
  };
  ...
  const enviarAuditoriaFinal = async () => {
    ...
    const supervisorReportPayload = {
      local_id: selectedLocal, chain_id: localData?.chain_id || null, cadena: selectedChain,
      start_time: visitStartTime, end_time: new Date().toISOString(), responses: answers,
      observations: generalObservations, photo_entry: fotoEntrada, photo_exit: fotoTermino 
    };
    try {
      await api.post(`/supervisor/audit`, supervisorReportPayload);
      toast.success("¡Registrado!", { id: toastId });
      setStep(4);
    } catch (err) { toast.error("Error al registrar", { id: toastId }); } finally { setLoading(false); }
  };
  ...
  const renderPhotoContainer = (photoUrl, setPhotoUrl, placeholderText) => {
    ...
    return (
      <div onClick={() => !capturing && fileInputRef.current.click()} className="w-full max-w-sm mx-auto aspect-square bg-gray-50 border-4 border-dashed border-gray-200 rounded-[3rem] flex flex-col items-center justify-center cursor-pointer active:scale-95">
        {capturing 
      ...
      <div className="w-full max-w-md md:max-w-xl bg-white p-6 md:p-10 rounded-[2.5rem] shadow-xl text-center space-y-6 border border-gray-100">
        ...
        {step === 2 && (
          <div className="space-y-5 animate-in slide-in-from-right duration-300 text-left">
            ...
            <button onClick={() => setStep(3)} className="w-full bg-black text-white py-5 md:py-6 rounded-[2.5rem] font-black uppercase text-[10px] md:text-xs">Siguiente</button>
          </div>
        )}
