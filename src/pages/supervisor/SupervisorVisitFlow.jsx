...
const SupervisorVisitFlow = () => {
  ...
  // Cadenas derivadas del campo "cadena" (string), chain_id viene null del backend
  const [chains, setChains] = useState([]);
  const [allLocals, setAllLocals] = useState([]);
  const [filteredLocals, setFilteredLocals] = useState([]);
  const [selectedChain, setSelectedChain] = useState(""); // valor = nombre cadena (string)
  ...
  const stepsInfo = {
    1: { key: "foto_entrada_supervisor", title: "Inicio de Visita", sub: "Selección de punto de venta" },
    2: { key: "encuesta_supervisor", title: "Auditoría de Sala", sub: "Cuestionario de supervisión" },
    3: { key: "observaciones_supervisor", title: "Cierre de Auditoría", sub: "Evidencia final y notas" },
    4: { key: "cierre_total", title: "Visita Sincronizada", sub: "Proceso completado" } 
  };
  ...
  useEffect(() => {
    const fetchSupervisorMasters = async () => {
      try {
        ...
        console.log("🔍 my-locales response:", JSON.stringify(myLocalsData, null, 2));
        setAllLocals(myLocalsData);

        // ✅ Usar campo "cadena" (string) como clave — chain_id viene null
        ...
        // Si solo hay un local, preseleccionar directamente
        ...
        toast.error("Error cargando tus locales");
      }
    };
    fetchSupervisorMasters();
  }, []);

  // Filtrar locales por nombre de cadena
  useEffect(() => {
    if (selectedChain) {
      const filtered = allLocals.filter(l => l.cadena === selectedChain);
      setFilteredLocals(filtered);
      // Auto-seleccionar si solo hay uno en esa cadena
      if (filtered.length === 1) {
        setSelectedLocal(filtered[0].id);
      } else if (!filtered.find(l => l.id === selectedLocal)) {
        setSelectedLocal("");
      }
    } else {
      setFilteredLocals([]);
  ...
  useEffect(() => {
    if (selectedLocal && !visitStartTime) {
  ...
  const handleCapture = async (e) => {
    ...
    try {
      ...
      if (step === 1) setFotoEntrada(photoPath);
      if (step === 3) setFotoTermino(photoPath);
  ...
  const enviarAuditoriaFinal = async () => {
    ...
    if (missingRequired) {
      toast.error("Faltan preguntas obligatorias");
      setStep(2);
      return;
    }
    if (!fotoTermino) {
      toast.error("Falta foto de término");
      return;
    }
    setLoading(true);
  ...
  const salirDeModulo = () => navigate("/supervisor/home");

  const renderPhotoContainer = (photoUrl, setPhotoUrl, placeholderText) => {
    if (photoUrl) {
      return (
        <div className="relative w-full aspect-square bg-gray-50 rounded-[3rem] overflow-hidden border-4 border-gray-100 shadow-xl">
          <img src={formatImageUrl(photoUrl)} className="w-full h-full object-cover" alt={placeholderText} />
    ...
    return (
      <div onClick={() => !capturing && fileInputRef.current.click()} className="w-full aspect-square bg-gray-50 border-4 border-dashed border-gray-200 rounded-[3rem] flex flex-col items-center justify-center cursor-pointer active:scale-95">
        {capturing 
  ...
  return (
    <div className={`min-h-screen p-4 pb-24 flex flex-col items-center ${isOnline ? 'bg-gray-50' : 'bg-orange-50/40'}`}>
      {/* Barra de progreso */}
      ...
      <div className="w-full max-w-md bg-white p-6 rounded-[2.5rem] shadow-xl text-center space-y-6 border border-gray-100">
        <div className="space-y-1 pt-2">
          <h2 className="text-xl font-black uppercase text-gray-900">{stepsInfo[step]?.title}</h2>
          <p className={`text-[9px] font-black uppercase tracking-[0.2em] ${isOnline ? 'text-[#87be00]' : 'text-orange-500'}`}>
            {stepsInfo[step]?.sub}
          </p>
        </div>

        {/* ── STEP 1: Selección de local ── */}
        {step === 1 && (
          <div className="space-y-5 animate-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-1 gap-3">

              {/* Select Cadena — value = nombre cadena string */}
              <select
                value={selectedChain}
                onChange={(e) => setSelectedChain(e.target.value)}
                className="w-full p-4 bg-gray-50 rounded-[1.5rem] text-xs font-bold shadow-inner"
              >
              ...
              <select
                ...
                className="w-full p-4 bg-gray-50 rounded-[1.5rem] text-xs font-bold shadow-inner disabled:opacity-40"
              >
                <option value="">Seleccione Local...</option>
                {filteredLocals.map(l => (
                  <option key={l.id} value={l.id}>
                    {l.codigo_local} — {l.direccion}{l.comuna_name ? ` (${l.comuna_name})` : ""}
                  </option>
                ))}
              </select>

              {/* Badge informativo del local seleccionado */}
              {localSeleccionado && (
                <div className="flex items-center gap-2 bg-[#87be00]/10 border border-[#87be00]/30 rounded-[1.2rem] px-4 py-3 text-left animate-in zoom-in duration-300">
                  <FiMapPin className="text-[#87be00] shrink-0" size={16} />
                  <div>
                    <p className="text-[10px] font-black uppercase text-[#87be00]">Local asignado</p>
                    <p className="text-xs font-bold text-gray-700">
                      {localSeleccionado.codigo_local} — {localSeleccionado.direccion}
                      {localSeleccionado.comuna_name ? `, ${localSeleccionado.comuna_name}` : ""}
                    </p>
                  </div>
                </div>
              )}
            ...
            {selectedLocal && (
              <div className="pt-2 animate-in zoom-in space-y-4">
                {renderPhotoContainer(fotoEntrada, setFotoEntrada, "Fachada")}
                {fotoEntrada && (
                  <button onClick={() => setStep(2)} className="w-full bg-[#87be00] text-white py-5 rounded-[2.5rem] font-black uppercase text-[10px]">
                    Comenzar
                  </button>
                )}
              </div>
        ...
        {step === 2 && (
          <div className="space-y-5 animate-in slide-in-from-right duration-300 text-left">
              ...
              {questions.map((q) => {
                ...
                return (
                  <div key={q.id} className="bg-gray-50/70 p-5 rounded-[2rem] border shadow-sm">
                    <p className="text-xs font-black uppercase">{q.question} {q.is_required && "*"}</p>
                    {isBoolean ? (
                      <div className="grid grid-cols-2 gap-4 pt-1">
                        <label className="flex items-center gap-3">
                          <input type="radio" checked={currentAnswer === "SI"} onChange={() => setAnswers({...answers, [q.id]: "SI"})} />
                          <span>Sí</span>
                        </label>
                        <label className="flex items-center gap-3">
                          <input type="radio" checked={currentAnswer === "NO"} onChange={() => setAnswers({...answers, [q.id]: "NO"})} />
                          <span>No</span>
                        </label>
                      </div>
                    ) : (
                      <input
                        ...
                        className="w-full bg-white border rounded-xl px-4 py-3 text-xs"
                      />
                    )}
                  </div>
                );
            ...
            <button onClick={() => setStep(3)} className="w-full bg-black text-white py-5 rounded-[2.5rem] font-black uppercase text-[10px]">
              Siguiente
            </button>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4 animate-in slide-in-from-right duration-300 text-left">
            <textarea
              ...
              className="w-full h-28 p-5 bg-gray-50 rounded-[2rem] text-xs font-bold"
            />
            {renderPhotoContainer(fotoTermino, setFotoTermino, "Término")}
            <button
              onClick={enviarAuditoriaFinal}
              disabled={loading || !fotoTermino}
              className="w-full bg-black text-white py-5 rounded-[2.5rem] font-black uppercase text-[10px] disabled:opacity-40"
            >
              Sincronizar
            </button>
          </div>
        ...
        {step === 4 && (
          <div className="py-6 space-y-4 animate-in zoom-in">
            <FiCheckCircle className="text-[#87be00] mx-auto" size={42} />
            <p className="text-xs font-bold uppercase">Auditoría Sincronizada</p>
            <button onClick={salirDeModulo} className="w-full bg-[#87be00] text-white py-6 rounded-[2.5rem] font-black uppercase text-xs">
              Volver
            </button>
          </div>
        )}
