      ...
      <div className="bg-white border-b border-gray-100 pl-[76px] pr-4 py-5 md:px-8 md:py-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 shrink-0">
        <div className="flex flex-col justify-center space-y-1">
          ...
          <div className="flex items-center gap-2 mt-1.5">
             ...
             <p className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">Auditoría por Ruta • {selectedDate}</p>
          </div>
        </div>

        <div className="flex flex-wrap gap-3 w-full lg:w-auto items-center mt-2 md:mt-0">
          ...
          <button onClick={() => setShowFilters(p => !p)} className={`p-4 rounded-[1.2rem] shadow-sm transition-all ${showFilters ? 'bg-[#87be00] text-white' : 'bg-white border border-gray-100 text-gray-400 hover:text-[#87be00]'}`}>
            <FiFilter size={18} />
          </button>
          <button onClick={fetchTasks} className="p-4 bg-gray-900 rounded-[1.2rem] text-white hover:bg-black shadow-md transition-all">
      ...
      <div className="p-4 md:p-8 flex-1 overflow-y-auto space-y-6 md:space-y-8 pb-10">
        ...
        <AnimatePresence>
          {showFilters && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 flex flex-wrap gap-4 items-center shadow-sm border border-gray-100">
                <select value={filterWorker} onChange={e => setFilterWorker(e.target.value)} className="flex-1 min-w-[200px] px-6 py-4 bg-gray-50 rounded-[1.5rem] text-[10px] font-black uppercase outline-none border border-gray-100 focus:border-[#87be00] transition-colors">
                  <option value="">Colaborador...</option>
                  {workers.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                </select>
                <select value={filterBrand} onChange={e => setFilterBrand(e.target.value)} className="flex-1 min-w-[200px] px-6 py-4 bg-gray-50 rounded-[1.5rem] text-[10px] font-black uppercase outline-none border border-gray-100 focus:border-[#87be00] transition-colors">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {[
            ...
            { label: "Prod. Revisados", value: rawTasks.length, color: "text-amber-500", bg: "bg-amber-50", icon: FiPackage },
          ].map((kpi, i) => (
            <motion.div key={i} whileHover={{ y: -5 }} className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-5 md:p-6 shadow-sm border border-gray-100 flex items-center gap-4">
              <div className={`w-12 h-12 md:w-14 md:h-14 ${kpi.bg} ${kpi.color} rounded-2xl flex items-center justify-center shrink-0`}>
                <kpi.icon size={20} className="md:w-6 md:h-6" />
              </div>
              <div className="min-w-0">
                <p className="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest">{kpi.label}</p>
                <h4 className="text-lg md:text-xl font-black text-gray-900 truncate mt-0.5">{loading ? "—" : kpi.value}</h4>
              </div>
            </motion.div>
          ))}
        </div>
                ...
                {groupedVisits.map((visit, idx) => (
                  <React.Fragment key={visit.id || idx}>
                    <motion.tr 
                      ...
                      className={`cursor-pointer transition-all text-center ${expandedRow === idx ? 'bg-gray-50' : 'hover:bg-gray-50/50 bg-white'}`}
                    >
                      <td className="px-8 py-5 text-left">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 bg-[#87be00] text-white rounded-2xl flex items-center justify-center text-xs font-black shadow-md shadow-[#87be00]/20 shrink-0">
                    ...
                    <AnimatePresence>
                      {expandedRow === idx && (
                        <motion.tr initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
                          <td colSpan={6} className="bg-gray-50/80 px-10 py-8 overflow-hidden">
